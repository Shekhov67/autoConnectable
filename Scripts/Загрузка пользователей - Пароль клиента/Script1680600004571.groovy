...
static def NewUser() {
    for (int i = 0; i < 355; ++i) {
        ...
        WebUI.setText(findTestObject('Page_Connectable/пароль'), findTestData('dataFile').getValue(8, 8 + i))

        WebUI.setText(findTestObject('Page_Connectable/подтверждение пароля'), findTestData('dataFile').getValue(8, 8 + 
                i))
